...
it('shows OS-protected secret status without requiring a SQLite password flag',async()=>{
  window.electronAPI.webdavSecretStatus.mockResolvedValue({success:true,available:true,stored:true,managed:true,backend:'dpapi'})
  api.mockImplementation(async(path)=>{if(path==='/api/settings')return{...settings,sync_enabled:true,sync_provider:'webdav',sync_endpoint:'https://dav.example.test/notepad',sync_password_set:false};if(path==='/api/sync/status')return{...status,provider:'webdav'};if(path==='/api/sync/conflicts')return[];return null})
  await render()
