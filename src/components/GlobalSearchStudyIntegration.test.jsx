...
it('batch workbench marks are visible in the original editor without opening the manuscript', async () => {
 const { entry, study } = await seedHubNote()
 vi.stubGlobal('navigator', { locks: { request: (_name, _options, fn) => Promise.resolve().then(fn) }, platform: 'Win32' })
 const before = await study.load(entry)
 await render(); await click(button('阅读批注')); await advance()
 await click(button('选择本页批注')); await change(label('批量阅读目标状态'), 'read')
 await click(button('预检批量标记')); await click(button('确认批量标记')); await advance()
 expect((await study.load(entry)).data.bookmark).toEqual(before.data.bookmark)
 await click(button('查看并编辑此批注')); await advance()
 expect(label('当前资料阅读状态').value).toBe('read'); expect(label('当前资料批注').value).toBe('集中查看末页批注')
 expect(onOpenFile).not.toHaveBeenCalled(); expect(onOpened).not.toHaveBeenCalled(); expect(onClose).not.toHaveBeenCalled()
})
it('batch preflight from the real search workspace leaves an existing editor draft untouched', async () => {
 await seedHubNote(); await render(); await click(button('阅读批注')); await advance()
 await click(button('查看并编辑此批注')); await advance()
 const input = label('当前资料批注')
 await act(async () => { Object.getOwnPropertyDescriptor(HTMLTextAreaElement.prototype, 'value').set.call(input, '批量时也不能丢的草稿'); input.dispatchEvent(new Event('input', { bubbles: true })); input.dispatchEvent(new Event('change', { bubbles: true })) })
 await click(button('阅读批注')); await advance(); await click(button('选择本页批注'))
 await change(label('批量阅读目标状态'), 'read'); await click(button('预检批量标记'))
 expect(container.textContent).toContain('所选条目有未保存批注'); expect(label('批量标记预检')).toBeNull()
 await click(button('查看并编辑此批注')); await advance()
 expect(label('当前资料批注').value).toBe('批量时也不能丢的草稿'); expect(onOpenFile).not.toHaveBeenCalled()
 const { collectionStudyDrafts } = await import('../services/collectionStudyDrafts')
 await act(async () => collectionStudyDrafts.list().forEach(({ key }) => collectionStudyDrafts.remove(key)))
})
