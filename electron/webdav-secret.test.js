...
describe('WebDAV OS-protected secret store',()=>{
  it('stores only encrypted bytes and can replace/load/clear the secret',async()=>{
    ...
    expect(await store.status()).toEqual({available:true,stored:true})
    ...
    await store.clear()
    expect(await store.status()).toEqual({available:true,stored:false})
  ...
  it('refuses persistence when OS encryption is unavailable',async()=>{
    ...
    expect(await store.status()).toEqual({available:false,stored:false})
