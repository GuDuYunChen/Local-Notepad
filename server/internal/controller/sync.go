...
func (c *SyncController) ConfigureAuto(r *ghttp.Request) {
	...
	if err := r.Parse(&input); err != nil { writeErr(r, 4009, "自动同步参数错误", err); return }
	value, err := c.Engine.ConfigureAuto(r.GetCtx(), input.Enabled, input.IntervalMinutes)
	if err != nil { writeErrWithDetail(r, 4010, "更新自动同步失败", err); return }
	writeOK(r, value)
}
