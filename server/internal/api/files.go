...
func (a *FileAPI) SaveAs(r *ghttp.Request) {
	...
	if err := r.Parse(&in); err != nil {
		writeErr(r, 1008, "参数错误", err)
		return
	}
	if in.Path == "" {
		writeErr(r, 1009, "路径不能为空", gerror.New("empty path"))
		return
	}
	if err := a.Svc.SaveAs(r.GetCtx(), id, in.Path, in.Encoding); err != nil {
		writeErr(r, 1010, "另存为失败", err)
		return
	}
	writeOK(r, g.Map{"ok": true})
}
...
func (a *FileAPI) Import(r *ghttp.Request) {
	...
	if in.Path == "" {
		writeErr(r, 1012, "路径不能为空", gerror.New("empty path"))
		return
	}
	f, err := a.Svc.ImportPath(r.GetCtx(), in.Path, in.Encoding)
	if err != nil {
		writeErr(r, 1013, "导入失败", err)
		return
	}
	writeOK(r, f)
}
